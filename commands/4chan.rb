class ChanCommand < SisterMercy::Command
  def get_board(board, page=1)
    get_json_from "https://api.4chan.org/#{board}/#{page}.json"
  end

  def get_post(board, thread_number)
    get_json_from "https://api.4chan.org/#{board}/res/#{thread_number}.json"
  end

  def get_raw_board(board, page=1)
    get_raw_json_from "https://api.4chan.org/#{board}/#{page}.json"
  end

  def get_raw_post(board, thread_number)
    get_raw_json_from "https://api.4chan.org/#{board}/res/#{thread_number}.json"
  end
